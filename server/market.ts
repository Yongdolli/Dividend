...
// Loads .env/.env.local for local runs; a no-op on Vercel where env vars
...
// ---------------------------------------------------------------------------
// Minimal request/response shapes satisfied by both Express and Vercel
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// In-memory caches. On serverless these live per warm instance — misses just
// mean an extra Yahoo round-trip, never stale or fabricated data.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// HTTP handlers — shared by the local Express server and Vercel functions
// ---------------------------------------------------------------------------

/** GET /api/fx — current KRW exchange rates */
export async function fxHandler(_req: HttpRequest, res: HttpResponse) {
  try {
...
/** GET /api/sync-recs — recommendation board with live prices/dividend data */
export async function syncRecsHandler(req: HttpRequest, res: HttpResponse) {
...
/** POST /api/refresh-quotes — refresh live prices for the user's holdings */
export async function refreshQuotesHandler(req: HttpRequest, res: HttpResponse) {
